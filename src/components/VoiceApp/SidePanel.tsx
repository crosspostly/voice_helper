...
export const SidePanel: React.FC = () => {
  ...
  return (
    <>
      {/* Desktop Side Panel */}
      <div className="hidden md:flex w-1/3 lg:w-1/4 bg-card border-r border-border p-6 flex-col space-y-6 overflow-y-auto shadow-sm">
        <SidePanelContent />
      </div>
      ...
      <div className={`md:hidden fixed top-0 left-0 h-full w-4/5 max-w-xs bg-card p-6 flex-col space-y-6 overflow-y-auto z-50 shadow-xl transform transition-transform duration-300 ${ui.isPanelVisible ? 'translate-x-0' : '-translate-x-full'}`}>
        <SidePanelContent />
      </div>
    </>
  );
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex items-center space-x-2">
          <button 
            ...
            className="p-2 rounded-lg hover:bg-grapefruit bg-white border-2 border-border transition-colors"
            aria-label="Settings"
          >
