...
export const SidePanel: React.FC = () => {
  ...
  return (
    <>
      {/* Desktop Side Panel */}
      <div className="hidden md:flex w-1/3 lg:w-1/4 bg-white border-r border-gray-300 p-6 flex-col space-y-6 overflow-y-auto shadow-sm">
        <SidePanelContent />
      </div>
      ...
      <div className={`md:hidden fixed top-0 left-0 h-full w-4/5 max-w-xs bg-white p-6 flex-col space-y-6 overflow-y-auto z-50 shadow-xl transform transition-transform duration-300 ${ui.isPanelVisible ? 'translate-x-0' : '-translate-x-full'}`}>
        <SidePanelContent />
      </div>
    </>
  );
  ...
  return (
    <>
      ...
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-bold text-gray-800">{t.personaTitle}</h2>
        <div className="flex items-center space-x-2">
          <button 
            onClick={() => ui.setIsSettingsModalOpen(true)} 
            className="p-2 rounded-lg hover:bg-gray-200 bg-white border-2 border-gray-300 transition-colors"
            aria-label="Settings"
          >
