...
export const TranscriptPanel: React.FC = () => {
  ...
  if (transcript.transcript.length === 0 && (status === 'IDLE' || status === 'ERROR')) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center text-center text-text opacity-60">
        <p className="mb-4">{t.startMessage}</p>
        <button 
          onClick={() => session.start(session.selectedAssistant!)} 
          className="bg-accent text-white p-6 rounded-full hover:bg-accent hover:opacity-90 transition-transform transform hover:scale-105 shadow-lg"
        >
  ...
  if (status === 'LISTENING' && transcript.transcript.length === 0) {
    return (
      <div className="flex-1 flex items-center justify-center">
        <p className="text-5xl font-bold text-accent animate-pulse">{t.speakNow}</p>
      </div>
    );
  }
      ...
      {transcript.canLoadMore && (
        <div className="text-center">
          <button 
            ...
            className="bg-card hover:bg-base text-sm font-semibold px-4 py-2 rounded-full transition-colors border border-border text-text"
          >
            {t.loadMore}
          </button>
        </div>
