...
export const ProgressCard: React.FC<ProgressCardProps> = ({
  ...
  const getDifficultyColor = (difficulty: Exercise['difficulty']) => {
    switch (difficulty) {
      case 'beginner':
        return 'bg-accent bg-opacity-20 text-accent border-accent border-opacity-30';
      case 'intermediate':
        return 'bg-yellow-100 text-yellow-800 border-yellow-200';
      case 'advanced':
        return 'bg-accent2 bg-opacity-20 text-accent2 border-accent2 border-opacity-30';
      default:
        return 'bg-grapefruit text-text border-border';
    }
  };
  ...
  return (
    <div className={`border border-border rounded-lg p-4 space-y-4 bg-grapefruit ${className}`}>
      ...
      {progressUpdates.length > 0 && (
        <div>
          <h4 className="font-semibold text-text mb-3 flex items-center">
            <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 mr-2 text-accent" fill="currentColor" viewBox="0 0 20 20">
            ...
            {progressUpdates.map((update, index) => (
              <div key={index} className="border-l-4 border-accent pl-4">
                <div className="flex justify-between items-start mb-1">
                  <h5 className="font-medium text-text">{update.category}</h5>
            ...
            {exercises.map((exercise, index) => (
              <div key={index} className={`border rounded-lg p-3 ${getDifficultyColor(exercise.difficulty)}`}>
                <div className="flex justify-between items-start mb-2">
                  <h5 className="font-medium text-text">{exercise.title}</h5>
